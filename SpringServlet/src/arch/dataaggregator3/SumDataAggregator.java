...
public class SumDataAggregator extends DataAggregator {
    ...
    public List<IDataItem> get() {
        ...
        IDataItem item = sumAll(itemsList);
        itemsList.clear();
        itemsList.add(item);
    ...
    private IDataItem sumAll(List<IDataItem> list) {
    	int sum = 0;
    	for(IDataItem item : list) {
    		sum += item.getRevenue();
    	}
    	return new DefaultDataItem("Sum", new Integer(sum));
    }
